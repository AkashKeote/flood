...
def get_weather_data(lat, lon):
    ...
    try:
        ...
        # Filter out None values from API response
        daily_precip = [p if p is not None else 0.0 for p in data["daily"]["precipitation_sum"]]
        daily_hours = [h if h is not None else 0.0 for h in data["daily"]["precipitation_hours"]]
        ...
        try:
            ...
            # Use forecast data values but assign to past dates, filter None values
            daily_precip = [p if p is not None else 0.0 for p in data["daily"]["precipitation_sum"][:7]]  # take only up to 7 days
            
            # Fill in missing data
            while len(daily_precip) < 7:
                daily_precip.append(0.0)
                
            # Calculate intensity from hourly data, filter None values
            intensity_per_day = []
            hourly_precip = [h if h is not None else 0.0 for h in data["hourly"]["precipitation"]]
            ...
            daily_hours = [min(24, max(0, int(p * 5))) if p is not None else 0 for p in daily_precip]
